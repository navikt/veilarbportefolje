...
public class PostgresServiceTest {
    ...
    @Test
    public void sok_pa_navn_case_insesitive() {
        when(veilarbVeilederClient.hentVeilederePaaEnhet(any())).thenReturn(List.of("Z12345", "Z12346"));
        leggTilOppfolgingsBruker("123", new OppfolgingsbrukerKafkaDTO().setFornavn("Test Abc").setEtternavn("Def"));
        leggTilOppfolgingsBruker("124", new OppfolgingsbrukerKafkaDTO().setFornavn("aBc").setEtternavn("Tom"));
        leggTilOppfolgingsBruker("125", new OppfolgingsbrukerKafkaDTO().setFornavn("Bac").setEtternavn("ABC"));

        Filtervalg filtervalg = new Filtervalg().setNavnEllerFnrQuery("abc");

        BrukereMedAntall brukere = postgresService.hentBrukere(enhetId, null, null, null, filtervalg, 0, 10);
        Assert.assertEquals(3, brukere.getAntall());
    }

    @Test(timeout = 100)
    public void sok_pa_kompleks_navn() {
        when(veilarbVeilederClient.hentVeilederePaaEnhet(any())).thenReturn(List.of("Z12345", "Z12346"));
        leggTilOppfolgingsBruker("123", new OppfolgingsbrukerKafkaDTO().setFornavn("Andre").setEtternavn("Abc"));
        leggTilOppfolgingsBruker("124", new OppfolgingsbrukerKafkaDTO().setFornavn("Navn Andre").setEtternavn("Test"));
        leggTilOppfolgingsBruker("125", new OppfolgingsbrukerKafkaDTO().setFornavn("Navn").setEtternavn("Andre"));

        Filtervalg filtervalg = new Filtervalg().setNavnEllerFnrQuery("Navn Andre");
        BrukereMedAntall brukere = postgresService.hentBrukere(enhetId, null, null, null, filtervalg, 0, 10);
        Assert.assertEquals(2, brukere.getAntall());
        Assert.assertTrue(brukere.getBrukere().stream().anyMatch(b -> b.getFornavn().equals("Navn Andre")));
        Assert.assertTrue(brukere.getBrukere().stream().anyMatch(b -> b.getFornavn().equals("Navn") && b.getEtternavn().equals("Andre")));
    }
    ...
    private void leggTilOppfolgingsBruker(String aktorIdString, OppfolgingsbrukerKafkaDTO oppfolgingsbrukerKafkaDTO) {
        AktorId aktorId = AktorId.of(aktorIdString);
        oppfolgingRepositoryV2.settUnderOppfolging(aktorId, ZonedDateTime.now());
        oppfolginsbrukerRepositoryV2.leggTilEllerEndreOppfolgingsbruker(oppfolgingsbrukerKafkaDTO.setAktoerid(aktorId.get()).setNav_kontor(enhetId).setEndret_dato(ZonedDateTime.now()));
    }
}
