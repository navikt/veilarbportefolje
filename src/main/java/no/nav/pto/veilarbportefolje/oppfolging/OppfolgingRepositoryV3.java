...
public class OppfolgingRepositoryV3 {
    private final JdbcTemplate db;

    public int settUnderOppfolging(AktorId aktoerId, ZonedDateTime startDato) {
        return db.update("""
                INSERT INTO oppfolging_data_v2 (AKTOERID, OPPFOLGING, STARTDATO) VALUES (?,?,?)
    ...
    public void settVeileder(AktorId aktorId, VeilederId veilederId) {
        db.update("UPDATE oppfolging_data_v2 SET veilederid = ? WHERE aktoerid = ?", veilederId.getValue(), aktorId.get());
    }

    public void settNyForVeileder(AktorId aktoerId, boolean nyForVeileder) {
        db.update("UPDATE oppfolging_data_v2 SET ny_for_veileder = ? WHERE aktoerid = ?", nyForVeileder, aktoerId.get());
    }

    public void settManuellStatus(AktorId aktoerId, boolean manuellStatus) {
        db.update("UPDATE oppfolging_data_v2 SET manuell = ? WHERE aktoerid = ?", manuellStatus, aktoerId.get());
    }

    public void settStartdato(AktorId aktoerId, ZonedDateTime startDato) {
        db.update("UPDATE oppfolging_data_v2 SET startdato = ? WHERE  aktoerid = ?", toTimestamp(startDato), aktoerId.get());
    }

    public void settSistTilordnetDato(AktorId aktoerId, ZonedDateTime sistTilordnetDato) {
        db.update("UPDATE oppfolging_data_v2 SET tilordnet_dato = ? WHERE aktoerid = ?", toTimestamp(sistTilordnetDato), aktoerId.get());
    }

    public void slettOppfolgingData(AktorId aktoerId) {
        db.update("DELETE FROM oppfolging_data_v2 WHERE aktoerid = ?", aktoerId.get());
    }
...
//                db.queryForObject("SELECT * FROM oppfolging_data_v2 WHERE aktoerid = ?", this::mapToBrukerOppdatertInformasjon, aktoerId.get())
//        ));
//    }
//
//
...
//                select aktoerid from oppfolging_data_v2 od
...
//        db.setFetchSize(10_000);
//        List<AktorId> alleIder = db.queryForList("SELECT aktoerid FROM oppfolging_data_v2 WHERE oppfolging", AktorId.class);
//        db.setFetchSize(-1);
//
//        return alleIder;
//    }
    ...
    public Optional<VeilederId> hentVeilederForBruker(AktorId aktoerId) {
        return Optional.ofNullable(
                queryForObjectOrNull(
                        () -> db.queryForObject("select veilederid from oppfolging_data_v2 where aktoerid = ?",
...
//        return db.query("select startdato, aktoerid from oppfolging_data_v2 where aktoerid = any (?::varchar[])",
