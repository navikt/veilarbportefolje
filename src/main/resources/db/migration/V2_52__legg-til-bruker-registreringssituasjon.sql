...
CREATE VIEW VW_PORTEFOLJE_INFO AS (
  SELECT
    ...
    BR.BRUKERSSITUASJON AS BRUKERSSITUASJON
  ...
  FROM
    ...
    LEFT JOIN BRUKER_REGISTRERING BR ON BRUKER_REGISTRERING.AKTOERID = MAP.AKTOERID
    LEFT JOIN (SELECT VEDTAKSTATUS, VEDTAK_STATUS_ENDRET_TIDSPUNKT, AKTOERID FROM VEDTAKSTATUS_DATA VD WHERE VEDTAKSTATUS = 'UTKAST_OPPRETTET' OR VEDTAKSTATUS = 'SENDT_TIL_BESLUTTER') VS ON VS.AKTOERID = MAP.AKTOERID
